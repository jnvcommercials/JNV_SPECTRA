...
const EventsCollection: React.FC = () => {
  ...
  const formattedEvents = events.map((event) => ({
    id: event.id,
    title: event.title,
    description: event.description, // or event.description if more suitable
  ...
  return (
    <section className="flex flex-col items-stretch max-w-7xl mx-auto px-4 pt-12">
      <header className="flex  justify-between items-stretch gap-5 text-black flex-wrap  max-md:max-w-full max-md:mr-0.5">
        <h2 className="text-[30px] font-medium max-md:text-[16px]">Event Planning</h2>
        <a href="#" 
        onClick={(e) => {
     ...
     {isLoading ? (
        ...
        <ServiceGrid services={formattedEvents} />
      )}
