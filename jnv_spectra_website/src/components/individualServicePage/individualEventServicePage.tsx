...
import { useEventServiceById } from "@/hooks/useEventServices";
import EventServiceDescription from "./eventServiceDescription";
...
const IndividualEventServicePage = () => {
  const { slug } = useParams<{ slug: string }>();
  const location = useLocation();
  const { id } = location.state as { id: string };
  // console.log(id);

  const { data: eventService, isLoading, error } = useEventServiceById(id);

  if (isLoading) { return(<div className="pt-40 flex flex-col items-center justify-center text-white text-lg animate-fadeIn">
  ...
  if (error || !eventService) {
    return <div className="pt-40 text-center">Failed to load service details.</div>;
  }
  ...
  return (
    <Layout>
      ...
      <div className="flex flex-col px-4 sm:px-6 lg:px-8 py-8 pt-40 max-w-7xl mx-auto w-full">
        ...
        <Gallery images={eventService.additional_images ?? []} />
      </div>
  ...
  bulletPoints={eventService.bullet_points ?? []}
/>
...
export default IndividualEventServicePage;
